...
contract BruRouter is Initializable, UUPSUpgradeable {
     ...
     * @param _tokenAmount The amount of tokens to deposit
     */
    function depositFromAdmin(
        uint256 _poolIndex,
        address _userAddress,
        address _tokenAddress,
        uint256 _tokenAmount
    ) external virtual {
        BruPool(poolShouldExist(_poolIndex)).deposit(_userAddress, _tokenAddress, _tokenAmount);
    }
     ...
     * @param _bondId The amount of tokens to withdraw
     */
    function withdrawFromAdmin(uint256 _poolIndex, uint256 _bondId, address _userAddress) external virtual {
        BruPool(poolShouldExist(_poolIndex)).withdraw(_userAddress, _bondId);
    }
