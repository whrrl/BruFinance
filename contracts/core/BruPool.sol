...
contract BruPool is BruAdmin, Initializable, ReentrancyGuard, UUPSUpgradeable {
    ...
    string[] private nftIds;
    uint256 private totalLoanAmount;
    uint256 private totalRepaidAmount;

    /**
     * @notice Add nft id into array for tracking nft data
     * @param _nftId of nft will be minted on chain
     */
    function addNftId(string memory _nftId) internal {
        nftIds.push(_nftId);
    }
    
    /**
     * @notice Get total number of nfts
     */
    ...
    /**
     * @notice Get all nft ids
     */
    function getNFTIds() public view returns(string[] memory) {
        return nftIds;
    }

    /**
     * @notice Get all nft details & total receipt value
     */
    function getNftData() public view returns(NFT[] memory, uint256){
        uint256 totalNFTs = getTotalNFTs();
        uint256 totalReceiptValue;
        NFT[] memory _nfts = new NFT[](totalNFTs);
        for(uint256 i = 0; i < totalNFTs; i++){
            _nfts[i] = nft[nftIds[i]];
            totalReceiptValue += nft[nftIds[i]].value;
        }
        return (_nfts, totalReceiptValue);
    }

    /**
     * @notice Get total loan amount
     */
    ...
    /**
     * @notice Get total repaid amount
     */
    function getTotalRepaidAmount() public view returns(uint256){
        return totalRepaidAmount;
    }

    /**
     * @notice Set borrow details for NFT
     * @param _userAddress of user who is holding NFT
     * @param _tokenAddress of borrowed token address
     * @param _nftId of of nft
     * @param _tokenAmount borrowed amount on NFT
     */
    function setBorrow(address _userAddress, address _tokenAddress, string memory _nftId, uint256 _tokenAmount) external onlyOwner{
        nft[_nftId].borrowed = true;
        assetLocked[_nftId] = true;
        borrowedNft[_nftId] = BorrowDetails(_tokenAmount, block.timestamp, _tokenAddress);
        totalLoanAmount += _tokenAmount;
        emit Borrowed(_tokenAmount, _nftId, block.timestamp, _userAddress, _tokenAddress);
    }

    /**
     * @notice Update Repay details for NFT
     * @param _userAddress of user who is holding NFT
     * @param _tokenAddress of borrowed token address
     * @param _nftId of of nft
     * @param _tokenAmount repaid amount on NFT
     */
    function setRepay(address _userAddress, address _tokenAddress, string memory _nftId, uint256 _tokenAmount) external onlyOwner{
        require(nft[_nftId].borrowed, "This NFT is not borrowed");
        borrowInterest(_nftId);
        uint256 amount = (_tokenAmount * (10 ** 18)) / ((10 ** 18) + platformFees.borrow);
        ...
        borrowedNft[_nftId].time = block.timestamp;
        totalRepaidAmount += _tokenAmount;

        if (borrowedNft[_nftId].borrowedAmount <= 0.1 ether) {
            nft[_nftId].borrowed = false;
            assetLocked[_nftId] = false;
            borrowedNft[_nftId] = BorrowDetails(0, 0, address(0));
        }
    ...
    /**
     * @notice Add nft id intin bulk for tracking nft data
     * @param _nftIds array of nft will be minted on chain
     */
    function addNftId(string[] memory _nftIds) external onlyOwner {
    ...
    /**
     * @notice Remove all nft ids
     */
    function removeNftIds() external onlyOwner {
        delete nftIds;
    }

     /**
     * @notice Get total receipt value
     */
    function getTotalReceiptValue() public view returns( uint256){
        uint256 totalNFTs = getTotalNFTs();
        uint256 totalReceiptValue;
        NFT[] memory _nfts = new NFT[](totalNFTs);
        for(uint256 i = 0; i < totalNFTs; i++){
